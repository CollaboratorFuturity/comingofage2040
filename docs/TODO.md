...
- [ ] **[Infra] Confirm host port + tunnel hostname.** Docker publishes on host port **3040** by default (`PORT` env var overrides, see docker-compose.yml). Confirm 3040 is free on the Debian server and add the hostname -> `http://localhost:3040` ingress rule to the Cloudflare tunnel config. Which subdomain will this live on?
